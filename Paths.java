    ...
    public static List<Node> shortestPath(Node start, Node end) {
        ...
        while (F.size != 0) {
        	...
        	for (Edge e : f.getExits()) {
        		w = e.getOther(f);
        		SFdata fInfo = info.get(f);
        		SFdata wInfo;
        		
        		if (!info.containsKey(w)) {
        			wInfo = new SFdata(fInfo.distance + e.length,f);
        			F.add(w, wInfo.distance);
        			info.put(w, wInfo);
        		}
        		
        		else if (fInfo.distance + e.length < info.get(w).distance) {
        			wInfo = info.get(w);
        			wInfo.distance = fInfo.distance + e.length;
        			F.changePriority(w, wInfo.distance);
        			wInfo.backPointer = f;
        		}
        	}
        }
